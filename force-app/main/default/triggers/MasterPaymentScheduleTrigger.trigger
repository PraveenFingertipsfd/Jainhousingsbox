trigger MasterPaymentScheduleTrigger on Master_Payment_Schedule__c (after update) {
    if (Trigger.isAfter && Trigger.isUpdate) {
        ...
        if (!projectWithSchedule.isEmpty()) {
            List<Booking__c> bookings = [SELECT Id, OwnerId FROM Booking__c WHERE Project__c IN :projectWithSchedule.keySet()];
            ...
            if (!bookings.isEmpty()) {
                ...
                //    BookingController.sendCustomNotification(userIds,'Master Payment Schedule Update',notificationsToSend.get(masterId),masterId,'Master_Payment_Schedule_Notification');
                }
            }
        }
    }
